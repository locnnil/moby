...
func Unpack(decompressedArchive io.Reader, dest string, options *TarOptions) error {
	...
	lg.Println()
	lg.Println()
	lg.Println("[DRI] Unpacking tar archive to", dest)

	lg.Println("[DRI] getting tar reader from decompressed archive")
	tr := tar.NewReader(decompressedArchive)

	var dirs []*tar.Header
	whiteoutConverter := getWhiteoutConverter(options.WhiteoutFormat)

	lg.Println("[DRI] Whiteout converter:", options.WhiteoutFormat)
	lg.Println("[DRI] ")
	lg.Println("[DRI] Iterating through tar archive files")
...
loop:
	for {
		...
		if err != nil {
			lg.Println("[DRI] Error reading tar header:", err)
			return err
		}
		...
		if err != nil {
			lg.Println("[DRI] Error creating implied directories:", err)
			return err
		}
		...
		if err != nil {
			lg.Println("[DRI] Error getting relative path: ", err)
			return err
		}
